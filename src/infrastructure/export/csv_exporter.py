"""
Exportation des donnees en format CSV.

Ce module fournit des fonctions pour exporter les resultats
de recherche en fichiers CSV.
"""
...
def export_pages_csv(
    ...
    Args:
        pages_final: Dictionnaire des pages avec leurs donnees.
        web_results: Resultats d'analyse web par page.
    ...
    with open(csv_path, "w", newline="", encoding="utf-8") as f:
        ...
        for pid, data in sorted(
            ...
            fb_link = (
                ...
                f"?active_status=all&ad_type=all&country={countries[0]}&view_all_page_id={pid}"
            )
...
def export_ads_csv(
    pages_for_ads: dict[str, Any],
    page_ads: dict[str, list[dict[str, Any]]],
    countries: list[str],
    output_dir: str = "resultats",
) -> tuple[str, int]:
    """
    ...
    Args:
        pages_for_ads: Dictionnaire des pages a exporter.
        page_ads: Annonces par page.
        countries: Liste des pays.
        output_dir: Repertoire de sortie.

    Returns:
        Tuple (chemin du fichier, nombre d'annonces exportees).
    """
    results_dir = Path(output_dir)
    results_dir.mkdir(exist_ok=True)
...
def export_suivi_csv(
    ...
    Args:
        ...
        web_results: Resultats d'analyse web.
        output_dir: Repertoire de sortie.
...
def create_dataframe_pages(
    pages_final: dict[str, Any],
    web_results: dict[str, Any],
    countries: list[str],  # noqa: ARG001
) -> pd.DataFrame:
    """
    Cree un DataFrame pandas pour les pages.

    Args:
        pages_final: Dictionnaire des pages.
        web_results: Resultats d'analyse web.
        countries: Liste des pays (non utilise actuellement).

    Returns:
        DataFrame avec les donnees des pages.
    """
    rows = []
