...
def run_search_process(keywords, countries, languages, min_ads, selected_cms, preview_mode=False):
    ...
    if db:
        try:
            from src.infrastructure.persistence.database import create_search_log, ensure_tables_exist
    ...
    # Récupérer les pages existantes en BDD (dernier scan < 1 jour)
    cached_pages = {}
    if db:
    ...
    print(f"[UI Search] Phase 4 - Filtre CMS (sélection: {selected_cms}):")
    ...
    # Log filtre min_ads
    pages_excluded_ads = {pid: data for pid, data in pages_with_cms.items() if data["ads_active_total"] < min_ads}
    print(f"[UI Search] Phase 5 - Filtre min_ads (min: {min_ads}):")
    print(f"   ✅ Pages retenues: {len(pages_final)}")
    print(f"   ❌ Pages exclues (< {min_ads} ads): {len(pages_excluded_ads)}")
    ...
    for pid, web_data in web_results.items():
        # Skip les pages en cache (déjà classifiées)
        if web_data.get("_skip_classification"):
            pages_skipped += 1
            continue
        # Classifier seulement si on a du contenu
    ...
    # Log détaillé pour debug classification
    print(f"[UI Search] Classification Debug:")
    ...
    if gemini_key and pages_to_classify_data:
        ...
        st.info(f"🏷️ Classification de {len(pages_to_classify_data)} pages avec Gemini...")
        ...
        st.warning("⚠️ Clé Gemini non configurée - classification ignorée")
                        ...
                        with col1:
                            ...
                            if new_thematique != current_thematique or new_classification != current_subcat:
                                if st.button("💾 Sauvegarder classification", key=f"save_class_{pid}"):
                                    from src.infrastructure.persistence.database import update_page_classification
...
# PAGE: BLACKLIST -> Migre vers pages/blacklist.py
...
# PAGE: FAVORIS -> Migre vers pages/favorites.py
# PAGE: COLLECTIONS -> Migre vers pages/collections.py
# PAGE: TAGS -> Migre vers pages/tags.py
...
# PAGE: SCHEDULED SCANS -> Migre vers pages/scheduled_scans.py
...
def render_search_logs():
    ...
    for log in logs:
        ...
        with st.expander(f"{status_emoji} **#{log_id}** - {date_str} - {keywords[:50]}{'...' if len(keywords) > 50 else ''} ({duration_str})"):
            ...
            from src.infrastructure.persistence.database import get_pages_for_search, get_winning_ads_for_search, get_search_history_stats, PageRecherche, WinningAds
